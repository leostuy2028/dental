...
# Paper Table 8.2 shows the position-balanced (shuffled) key only, which is the project
# default (RESEARCH_PLAN §1.0), and gives each run a readable name. Anything not listed here
# still appears in the terminal readout and the CSV; it just does not enter the paper table.
PAPER_ROWS = [
    ("gemini-2.5-flash__coax-direct-k0__shuffled__n491", "Gemini-2.5-flash"),
    ("gpt-4o-2024-11-20__coax-direct-k0__shuffled__n491", "GPT-4o"),
    ("gemini-3.5-flash__coax-direct-k0__shuffled__n491", "Gemini-3.5-flash"),
    ("gemini-3.5-flash__coax-direct-ctx-opgprimer__shuffled__n491", "Gemini-3.5-flash + primer"),
    ("gemini-3.5-flash__coax-direct-primerv1+visualex-v2__shuffled__n491",
     "Gemini-3.5-flash + primer + examples"),
]


def emit_paper_table(res, n_routed, n_total):
    """Table 8.2 as a markdown fragment + values.json, so the draft never carries a
    hand-typed number (RESEARCH_PLAN §1.0 rule 7)."""
    import json
    out_dir = os.path.join("paper_analysis", "_generated")
    os.makedirs(out_dir, exist_ok=True)
    stamp = datetime.datetime.now(datetime.timezone.utc).isoformat(timespec="seconds")
    by_run = {r.run: r for _, r in res.iterrows()}
    lines = [f"<!-- GENERATED by detector/eval_router.py on {stamp}. "
             f"Do not hand-edit; run `python detector/eval_router.py`. -->",
             "| Base model | alone | with the detector in front | change |",
             "|:--|--:|--:|--:|"]
    vals = {"n_routed": int(n_routed), "n_closed": int(n_total), "rows": {}}
    for key, label in PAPER_ROWS:
        r = by_run.get(key)
        if r is None:
            lines.append(f"| {label} | — | — | *(run not found)* |")
            continue
        lines.append(f"| {label} | {r.base:.1f}% | {r.routed:.1f}% | **{r.lift:+.1f}** |")
        vals["rows"][label] = {"run": key, "base": round(r.base, 2),
                               "routed": round(r.routed, 2), "lift": round(r.lift, 2),
                               "model_on_routed": round(r.model_on_routed, 1),
                               "detector_on_routed": round(r.det_on_routed, 1)}
    frag = "\n".join(lines) + "\n"
    open(os.path.join(out_dir, "router_lift_table.md"), "w", encoding="utf-8").write(frag)
    vals.update({"_generator": "detector/eval_router.py", "_generated_utc": stamp})
    json.dump(vals, open(os.path.join(out_dir, "router_lift.values.json"), "w"), indent=2)
    print("\n" + frag)
    print(f"wrote {out_dir}/router_lift_table.md and .values.json")
