...
| **§5.3.1** (**Table 5.3**) GPT-4o prompt×key 2×2: accuracy of original (faithful) and revised (coax) prompts on the original vs shuffled key (491), the two per-prompt drops, and the revised−original gap (+11.0 → +8.4) | `paper_analysis/shuffle_drop.py` | all four committed 491 CSVs: `…faithful…whole`, `…faithful…shuffled`, `…coax…whole`, `…coax…shuffled` (whole under `results/closed_ended/`, shuffled under `results/closed_ended/position_bias/`) | `_generated/shuffle_drop_table.md`, `_generated/shuffle_drop.values.json` |
