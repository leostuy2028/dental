...
| `closed_ended/knowledge_context/gemini-3.5-flash__coax-direct-{noctx,ctx-opgprimer}__e11sel__n130.csv` | E11 (knowledge-context screen) | §5.4 | gemini-3.5-flash | coax direct k=0 think0; ctx arm prepends `reference/opg_primer.txt` (v1) | enriched selection (100 misses + 30 controls) on the shuffled key, `data/closed_ended_e11_sel.parquet` | 130 | 2026-07-07 | `python eval_closed_gemini.py --model gemini-3.5-flash --prompt coax --thinking-budget 0 --data data/closed_ended_e11_sel.parquet [--context reference/opg_primer.txt] --out …`. Subset built by `python dataio/make_e11_subset.py` (parquet gitignored, regenerable). no-ctx 23.1% → +primer 40.8%; rescued 26/broke 3 (McNemar p<0.001); rescue 26% vs break 10%. Analyzer `paper_analysis/knowledge_context.py`; selection labels `knowledge_context/e11_selection.csv`. |
| `closed_ended/knowledge_context/gemini-3.5-flash__coax-direct-primerv1+visualex-v2__e11sel__n130.csv` | §5.4 primer+visual-exemplars (hard core) | §5.4 | gemini-3.5-flash | coax direct k=0 think0; prepends `reference/opg_primer.txt` **and** the 12 combined visual examples `reference/exemplars_v2.json` (6 DENTEX + 6 Zenodo) | same E11 enriched slice, `data/closed_ended_e11_sel.parquet`, shuffled key | 130 | 2026-07-08 | `python eval_closed_gemini.py --model gemini-3.5-flash --prompt coax --thinking-budget 0 --data data/closed_ended_e11_sel.parquet --context reference/opg_primer.txt --visual-exemplars reference/exemplars_v2.json --out …`. vs primer-only 40.8% → **49.2%**; rescued 18/broke 7 (McNemar p=0.043); gain concentrated HisT/Jaw (union 36.4→53.2). |
| `closed_ended/knowledge_context/gemini-3.5-flash__coax-direct-ctx-opgprimer__shuffled__n491.csv` | §5.4 primer, FULL benchmark | §5.4 | gemini-3.5-flash | coax direct k=0 think0 temp0 full-res; prepends `reference/opg_primer.txt` | **all 491**, shuffled key `data/closed_ended_shuffled.parquet` | 491 | 2026-07-08 | `python eval_closed_gemini.py --model gemini-3.5-flash --prompt coax --thinking-budget 0 --data data/closed_ended_shuffled.parquet --context reference/opg_primer.txt --out …` (see `run_s54_full.sh`). Baseline no-primer = `position_bias/…k0__shuffled__n491.csv` 58.04%; +primer **60.29%** (net +11, p=0.31). |
| `closed_ended/knowledge_context/gemini-3.5-flash__coax-direct-primerv1+visualex-v2__shuffled__n491.csv` | §5.4 primer+visual-exemplars, FULL benchmark | §5.4 | gemini-3.5-flash | coax direct k=0 think0 temp0 full-res; prepends `reference/opg_primer.txt` **and** `reference/exemplars_v2.json` (12 external examples) | **all 491**, shuffled key | 491 | 2026-07-08 | `python eval_closed_gemini.py --model gemini-3.5-flash --prompt coax --thinking-budget 0 --data data/closed_ended_shuffled.parquet --context reference/opg_primer.txt --visual-exemplars reference/exemplars_v2.json --out …` (see `run_s54_full.sh`). **62.73%**; vs no-primer net +23 (66 rescued/43 broke, p=0.035); Jaw 68.2→77.5. §5.4 table + paired stats regenerated by `paper_analysis/section54_table.py`. |
| `closed_ended/knowledge_context/gemini-3.5-flash__coax-direct-ctx-opgprimer__whole__n491.csv` | §5.4 primer only, ORIGINAL benchmark key (reference cell) | §5.4 | gemini-3.5-flash | coax direct k=0 think0 temp0 full-res; prepends `reference/opg_primer.txt` | **all 491**, ORIGINAL (B-skewed) key `data/closed_ended.parquet` | 491 | 2026-07-08 | `python eval_closed_gemini.py --model gemini-3.5-flash --prompt coax --thinking-budget 0 --data data/closed_ended.parquet --context reference/opg_primer.txt --out …` (from `run_s54_benchkey.sh`). **62.53%** (vs no-primer original-key 59.88% = `position_bias/…__whole__n491.csv`; primer shuffled 60.29%). Complete/valid; finished headless after the benchkey wrapper was stopped (the primer+v2 original-key arm was cancelled, not this one). Original-key = leaderboard-comparable but B-skew-contaminated; the honest number is the shuffled cell above. |
...
| `open/coordarms_gpt4o_cpc_all_{answers,scores}.csv` (+`.meta.json`) | E-open-coord (whole set) | §6.1/§6.2 | gpt-4o (answerer + judge) | arm `coax_primer_coords` = never-refuse persona + `reference/opg_primer.txt` + coordinate-JSON elicitation; temp=0, img_detail=**high**, max_tokens=2048 (verified non-binding), grader=rubric `original` (verbatim VLMEvalKit, **UNCHANGED**) | all **578** open-ended (`data/open_ended.parquet`) | 578 | 2026-07-09 | `python -m eval_open.run_coord_arms --n 578 --ref-type all --arms coax_primer_coords --tag gpt4o_cpc_all --model gpt-4o --judge gpt-4o`. Overall **36.5%** (coord_ref 37.1 / prose_ref 36.3); **0 refusals**; 550/578 emitted boxes. Coordinate accuracy `python -m paper_analysis.coord_iou …_answers.csv` → mean IoU **0.04**, median 0, 77% miss → coordinates bluffed (§6.1). |
| `open/coordarms_gpt4o_n30_{answers,scores}.csv` | E-open-coord (pilot) | §6.1/§6.2 | gpt-4o (answerer + judge) | 4 arms `plain→coax→coax_primer→coax_primer_coords`; temp=0, img_detail=**auto** (pilot; superseded by the high-detail full run for the combined number), grader `original` UNCHANGED | 30 coord_ref items (seed=0) | 30 | 2026-07-09 | `python -m eval_open.run_coord_arms --n 30 --tag gpt4o_n30 --model gpt-4o --judge gpt-4o`. Arm scores plain **16.3** / coax **39.3** / coax_primer **38.0** / coax_primer_coords **44.3%**. Deltas: refusal (coax−plain) **+23.0** p=0.0006; primer −1.3 p=0.92; coords **+6.3** p=0.41. Source of the pilot refusal & coordinate deltas cited in §6.2. |
| `open/batched_gpt5mini_{answers,scores}.csv` | E-open-model (gpt-5-mini) | §6.1 | gpt-5-mini (answerer) + gpt-4o (judge) | image-BATCHED (one call per unique image, all its questions): coax persona + `reference/opg_primer.txt`, **no coordinates**; reasoning_effort=minimal, img_detail=high, max_completion_tokens=12000; grader rubric `original` UNCHANGED | all **578** open (`data/open_ended.parquet`) | 578 | 2026-07-09 | `python -m eval_open.run_batched --model gpt-5-mini --provider openai --n-images 100 --tag gpt5mini`. Overall **48.5%** (prose 48.0 / coord 50.7); +12 vs gpt-4o (NOT prompt-matched). Feeds `paper_analysis/open_model_table.py`. |
| `open/batched_gpt5mini_ex_{answers,scores}.csv` | E-open-model (gpt-5-mini + exemplars) | §6.1 | gpt-5-mini + gpt-4o judge | batched, as above **plus** the 12 §5.4 visual exemplars (`reference/exemplars_v2.json`), no coordinates | all **578** open | 578 | 2026-07-09 | `python -m eval_open.run_batched --model gpt-5-mini --provider openai --exemplars reference/exemplars_v2.json --n-images 100 --tag gpt5mini_ex`. Overall **46.2%**; paired vs no-exemplars **−2.2** (p=0.11), pathology −7.7, jaw +1.6 → exemplars do not help on the free-text half. Feeds `paper_analysis/open_model_table.py`. |
| `open/batched_gpt5mini_ex_high_{answers,scores}.csv` (+`.meta.json`) | E-open-model (gpt-5-mini +exemplars, HIGH reasoning) | §6.1 | gpt-5-mini + gpt-4o judge | batched (8 workers), coax + primer + 12 exemplars, no coords, **reasoning_effort=high**, max_completion_tokens=32000; grader `original` UNCHANGED | all **578** open | 578 | 2026-07-09 | `python -m eval_open.run_batched --model gpt-5-mini --provider openai --exemplars reference/exemplars_v2.json --reasoning-effort high --workers 8 --n-images 100 --tag gpt5mini_ex_high`. Overall **46.3%**; reasoning effect vs +ex/minimal **+0.1** (Wilcoxon p=0.84) → raising a reasoning model minimal→high does NOT help the free-text half (perception-limited, not reasoning-limited). Isolates reasoning (both arms carry exemplars). |
| `open/localize_detection.csv` | E-open-detector (LOCALIZE ± text-map) | §8.1 | gpt-5-mini + gpt-4o judge | 49 teeth "which tooth" items; control = reused `batched_gpt5mini` baseline, +det = same batched call with a TEXT tooth-map (`reference/teeth_detections.json`, made by `eval_open/detect_teeth.py`) injected | 49 LOCALIZE | 49 | 2026-07-10 | `python -m eval_open.test_localize_detection`. Text map **HURT**: 35.9→29.4% (−6.5, p=0.39); maps 96% contain the right tooth yet correct-tooth naming fell 58→52% → model can't use a text map (image→text-space join fails). |
| `open/localize_detection_gemini35.csv` | E-open-detector (LOCALIZE ± text-map, gemini) | §8.1 | gemini-3.5-flash + gpt-4o judge | same 49 LOCALIZE, BOTH arms fresh | 49 | 2026-07-10 | `python -m eval_open.test_localize_detection --model gemini-3.5-flash --provider gemini --out …_gemini35.csv`. Baseline **40.8%** (correct-tooth 67%) > gpt-5-mini 35.9%/58% (localization improves with model strength, §8.1). Text map HURT here too: 40.8→34.3 (−6.5). Visual numbered-overlay (ad-hoc, not committed): idx3 0.0→0.9 proof, but VLM/hand overlays too imprecise (5-img 22→18%) → needs accurate detector (RESEARCH_PLAN NEXT ACTION). |
| `open/batched_gemini35_precision100_{answers,scores}.csv` | E-open-precision-probe (**NEGATIVE**) | not in paper | gemini-3.5-flash + gpt-4o judge | batched, coax + primer, thinking 4000, **`--precision`** (`PRECISION_NOTE`: be selective on summary/report Qs only). **PARTIAL 33-image probe** (full run interrupted — user did not want 100 img); answers graded post-hoc with the standard GPT-4o judge (rubric `original`, same as every open row) | 33 imgs / 190 items (33 Report, 34 Jaw, 123 other) | 2026-07-18 | `python -m eval_open.run_batched --model gemini-3.5-flash --provider gemini --thinking-budget 4000 --precision --n-images 100 --workers 8 --tag gemini35_precision100` (killed at 33 imgs). Paired vs `batched_gemini35_plain578`: Report **+0.6** [−4.7,+5.9] (flat, target unmoved), Jaw +4.4 [−4.0,+12.9]; SPILLOVER regressed — Teeth **−6.0 [−11.2,−0.8]**, COUNT −13, Patho −7, net **−2.7**. Conclusion: prompt-tuning gemini toward gpt's summary style is a dead end; do not retry. |
