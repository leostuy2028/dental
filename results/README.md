...
**E-prompt-axis (§5.2), parsing-vs-prompting:** faithful vs coax prompt for GPT-4o on the clean 453 (paired). Benchmark prompt read by the **benchmark's parser** = **44.4% [39.9–49.0]** (what the leaderboard reports); benchmark prompt read by the model's **true** (hand-verified) answer = **49.9% [45.3–54.5]** — the benchmark's parser misreads verbose replies on **49/453** items, costing ~5.5 pts; coax prompt read by the benchmark's parser = **53.6% [49.0–58.2]** (453/453 bare replies, **0** random fallbacks). Combined lift **+9.2** = ~5.5 parser + ~3.7 real prompt; McNemar p **1.6e-04** (coax-right 80 / faithful-right 38); coax refusals 0%. **Decision: coax prompt + keep the benchmark's own parser for all MCQ work.** Generator `paper_analysis/faithful_true_accuracy.py` (+ committed `faithful_hand_labels.csv`) → `_generated/faithful_true_accuracy_table.md`.
